...
const HeroInnerBanner = ({ show = true, onClose }) => {
	return (
		<>
			{show && (
				<div className='relative rounded bg-gradient-to-r from-purple-500 to-indigo-800 py-3 px-8 text-white shadow-lg md:rounded-full'>
					<p>
						Welcome to <span className='font-bold'>JEDE</span>. An easier and
...
const SearchForm = () => {
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			if (!isDirty) {
				const value = items[index >= items.length ? (index = 0) : index];
				setQuery(value);
			}
			index++;
		}, 3000);
		...
		>
			...
			>
				...
				<input
					...
					className='w-full appearance-none border-transparent p-2 text-xl leading-6 text-slate-900 placeholder-slate-400 focus:border-transparent focus:ring-0 dark:bg-white'
					onClick={() => {
						if (!isDirty) {
							setIsDirty(true);
							setQuery('');
